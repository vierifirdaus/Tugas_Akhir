...
import { BackendResultType, ParsedFlowResult, CustomNode, MethodNodeInfo, BackendClassItem, BackendMethodObject } from '@/types';
 ...
 */
export function parseBackendResultToFlow(result: BackendResultType): ParsedFlowResult {
  const nodesArray: CustomNode[] = [];
  const edgesArray: any[] = []; // Belum ada edges yang di-generate di kode asli
  let currentYPosition = 50; // Posisi Y awal untuk grup kelas pertama

  result.forEach((classItem: BackendClassItem) => {
