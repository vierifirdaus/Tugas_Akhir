...
export function parseCFG(classResult: BackendClassResultType, functionResult: BackendFunctionResultType, mainResult: string): ParsedFlowResult {
  ...
  classResult.forEach((classItem: BackendClassItem) => {
    ...
    for (let i = 0; i < methodNodesInfo.length; i += Math.floor(Math.sqrt(methodNodesInfo.length))) {
      const row = methodNodesInfo.slice(i, i + Math.floor(Math.sqrt(methodNodesInfo.length)));
      rows.push(row);
    }

    const rowWidths = rows.map(row => 
      row.reduce((sum, method) => sum + method.nodeWidth, 0) + (row.length > 1 ? METHOD_SPACING_HORIZONTAL : 0)
    );
    ...
    const totalContentHeight = rowMaxHeights.reduce((sum, height) => sum + height, 0) + (rows.length > 1 ? (rows.length - 1) * CLASS_SPACING_VERTICAL / 2 : 0);
    
    const finalGroupWidth = Math.max(MIN_GROUP_WIDTH, maxRowWidth + (GROUP_INTERNAL_PADDING_HORIZONTAL * 2));
    const finalGroupHeight = CLASS_LABEL_HEADER_HEIGHT + totalContentHeight + (GROUP_INTERNAL_PADDING_VERTICAL * 2);
    ...
    let yOffsetForRow = CLASS_LABEL_HEADER_HEIGHT + GROUP_INTERNAL_PADDING_VERTICAL;
